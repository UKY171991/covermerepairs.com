...
class Part_model extends CI_Model { 
	...
	{
		$this->db->select('part.*, brand.name as brand_name, model.name as model_name, part_type.name as part_type_name, user.name as user_name, branch_user.name as branch_name');
		$this->db->from('part');
		...
		$this->db->join('user as branch_user', 'branch_user.id = part.branch', 'left');
		$this->db->where($where);
		$query = $this->db->get();
		return $query->row();
	}
	...
	public function single_data_join_brand($where)
	{
		$this->db->select('brand.*, user.name as user_name, user.type as user_type');
		$this->db->from('brand');
		$this->db->join('user', 'user.id = brand.added_by', 'left');
		
		if(isset($where['brand.id'])){
			$this->db->where('brand.id', $where['brand.id']);
		}
	...
	public function single_data_join_part_type($where)
	{
		$this->db->select('part_type.*, user.name as user_name, user.type as user_type');
		$this->db->from('part_type');
		$this->db->join('user', 'user.id = part_type.added_by', 'left');
		
		if(isset($where['part_type.id'])){
			$this->db->where('part_type.id', $where['part_type.id']);
		}
