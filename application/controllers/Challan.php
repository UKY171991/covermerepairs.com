...
class Challan extends CI_Controller {
	...
	{
		$data['branch'] = $this->challan->all_data('branch','DESC');
		$data['ajax']='challan';
		$this->load->view('inc/header');
		$this->load->view('challan/all',$data);
	...
	public function add_data(){
		...
		if($this->input->post()){
			...
			if($id !=''){
				$this->challan->update('challan',$prem,$id);
				$mes=array('status'=>"success",'message'=>'Challan updated succefully.');
				echo json_encode($mes);
				exit();
			}else{
				$last_id = $this->challan->insert('challan',$prem);
				$mes=array('status'=>"success",'message'=>'Challan added succefully.');
				echo json_encode($mes);
				exit();
			}
	...
	public function all_data_ajax(){
		...
		$all_data = $this->challan->all_data('challan','DESC',$wher);
		...
		foreach($all_data as $key => $all_datas){
			$row = array(); 
				...
				$action .= "<a href='".base_url('challan/print/'.$all_datas->id)."' target='_blank' class='btn btn-primary btn-xs m-1'><i class='fa fa-print' aria-hidden='true'></i></a>";
			...
			$branch_data = $this->challan->all_data('branch','DESC',$wher);
		...
		$output = array(
                   // "draw" 				=> intval($_POST['draw']),
                    "recordsTotal" 		=> $this->challan->count_all('challan','DESC',$wher),
                    "recordsFiltered" 	=> $this->challan->count_all('challan','DESC',$wher),
                    "data" 				=> $data,
            	);
	...
	public function delete(){
		$id = $this->input->post('id');
		$this->challan->delete('challan',$id);
		echo "Challan deleted succefully.";
		exit();
	}
	...
	public function print($id){
		$data['ajax']='none';
		$wher = array('id'=>$id);
		$data['challan'] = $this->challan->all_data('challan','DESC',$wher);

		$this->load->view('inc/header');
		$this->load->view('challan/print',$data);
		$this->load->view('inc/footer');
	}
}
